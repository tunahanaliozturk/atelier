...
3. Write the mission file (mission-tracking skill) with all tasks `pending` and present the
   plan plus the task-to-agent assignments to the user. Stop and wait for explicit approval.
...
5. Dispatch with the dispatching-agents skill; verify each result with the
   verifying-task-output skill; keep the mission file current throughout. When a task fails
   verification, follow the verifying-task-output skill: send it back to its agent or mark
