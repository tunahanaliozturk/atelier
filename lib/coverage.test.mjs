...
test('coverageReport groups a cross-cutting agent without listing it as a gap', () => {
  const agents = [
    { name: 'lead', layer: 'orchestration', capabilities: ['routing'], task_kinds: ['coordinate'] },
  ];
  const report = coverageReport(agents);
  assert.ok(report.layers.orchestration, 'orchestration layer should be grouped');
  assert.deepEqual(report.layers.orchestration.agents, ['lead']);
  assert.ok(!report.gaps.includes('orchestration'), 'cross-cutting layer must never be a gap');
});
